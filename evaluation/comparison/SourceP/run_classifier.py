...
from transformers import (
    AdamW, get_linear_schedule_with_warmup, RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer
    )
...
class RuntimeContext(object):
    ...
    def __init__(self):
        ...
        self.train_data_path = config_parser.get(file_section, "train_data_path")
        self.eval_data_path = config_parser.get(file_section, "eval_data_path")
        self.test_data_path = config_parser.get(file_section, "test_data_path")
        self.model_check_point = config_parser.get(file_section, "model_check_point")
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results['eval_f1'] >= best_f1:
                        ...
                        if not os.path.exists(os.path.dirname(args.model_check_point)):
                            os.makedirs(args.model_check_point)
                        model_to_save = model.module if hasattr(
                            model, 'module') else model
                        torch.save(model_to_save.state_dict(), args.model_check_point)
                        logger.info(
                            "Saving model checkpoint to %s", args.model_check_point)


def evaluate(args, model, tokenizer, eval_when_training=False):
    # build dataloader
    if not os.path.exists(args.eval_data_path):
        if not os.path.exists(os.path.dirname(args.eval_data_path)):
            os.makedirs(os.path.dirname(args.eval_data_path))
        eval_dataset = TextDataset(tokenizer, args, file_path=args.eval_data_file)
        torch.save(eval_dataset, args.eval_data_path)
...
def test(args, model, tokenizer, best_threshold=0):
    ...
    if not os.path.exists(args.test_data_path):
        if not os.path.exists(os.path.dirname(args.test_data_path)):
            os.makedirs(os.path.dirname(args.test_data_path))
        eval_dataset = TextDataset(tokenizer, args, file_path=args.test_data_file)
        torch.save(eval_dataset, args.test_data_path)
        
    else:
        eval_dataset = torch.load(args.test_data_path)
...
def main():
    ...
    model = Model(pretrained_model, config, tokenizer, args)
    if not os.path.exists(args.train_data_path):
        if not os.path.exists(os.path.dirname(args.train_data_path)):
            os.makedirs(os.path.dirname(args.train_data_path))
        train_dataset = TextDataset(
            tokenizer, args, file_path=args.train_data_file
        )
        torch.save(train_dataset, args.train_data_path)

    else:
        train_dataset = torch.load(args.train_data_path)

    
    if os.path.exists(os.path.dirname(args.model_check_point)):
        if os.path.exists(args.model_check_point):
            model.load_state_dict(torch.load(args.model_check_point))

    else:
        os.makedirs(os.path.dirname(args.model_check_point))
